...
class FakeSwapClient:
    ...
    def postBid(self, offer_id, amount, addr_send_from=None, extra_options={}):
        self.post_options.append(extra_options)
...
class TestPlaceMultiBid(unittest.TestCase):
    ...
    def test_passes_each_led_leg_its_reserved_inputs(self):
        swap_client = self.leader_client(ltc(10), ltc(10))

        placed, _, _ = placeMultiBid(swap_client, self.legs(swap_client))

        self.assertEqual(
            [options["reserved_prevouts"] for options in swap_client.post_options],
            [json.loads(swap_client.kv[f"bid_prevouts:{p['bid_id']}"]) for p in placed],
        )
    ...
    def test_leaves_each_led_leg_room_for_change(self):
        # At 100 sat/vB the fake sizes give a lock fee of 20000 and an input 6800.
        book = [
            FakeOffer(b"A", 4, 0.42, min_bid_amount=0.1, b_fee_rate=100_000),
            FakeOffer(b"B", 9, 0.43, amount_negotiable=False, b_fee_rate=100_000),
        ]
        lock_only = [ltc(0.42) + 20000, ltc(3.87) + 20000]
        swap_client = FakeSwapClient(book)
        swap_client.ci(Coins.LTC).outputs = outs(*lock_only)

        placeMultiBid(swap_client, self.legs(swap_client))

        self.assertEqual(
            swap_client.ci(Coins.LTC).split_calls,
            [[value + 6800 for value in lock_only]],
        )
    ...
    def test_does_not_preselect_when_the_bidder_follows(self):
        ...
        self.assertFalse(
            any("reserved_prevouts" in options for options in swap_client.post_options)
        )
