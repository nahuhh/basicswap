...
"""
basicswap]$ export PARTICL_BINDIR=/tmp/partbuild/src; export DASH_BINDIR=~/tmp/dashcore-0.14.0/bin/ export BITCOIN_BINDIR=~/tmp/bitcoin-0.18.0/bin/
basicswap]$ export PYTHONPATH=$(pwd)
basicswap]$ python tests/basicswap/test_dash.py
...
DASH_BINDIR = os.path.expanduser(os.getenv('DASH_BINDIR', ''))
DASHD = os.getenv('DASHD', 'dashd' + ('.exe' if os.name == 'nt' else ''))
DASH_CLI = os.getenv('DASH_CLI', 'dash-cli' + ('.exe' if os.name == 'nt' else ''))
DASH_TX = os.getenv('DASH_TX', 'dash-tx' + ('.exe' if os.name == 'nt' else ''))
...
def dashRpc(cmd):
    return callrpc_cli(DASH_BINDIR, os.path.join(cfg.DATADIRS, str(DASH_NODE)), 'regtest', cmd, DASH_CLI)
...
def run_loop(self):
    while not stop_test:
        ...
        for c in self.swap_clients:
            c.update()
        dashRpc('generatetoaddress 1 {}'.format(self.dash_addr))
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        prepareOtherDir(cfg.DATADIRS, DASH_NODE)
        ...
        cls.daemons.append(startDaemon(DASH_NODE, DASH_BINDIR, DASHD))
    ...
    def test_07_error(self):
        ...
        swap_clients[0].coin_clients[Coins.DASH]['override_feerate'] = 10.0
