...
class BCHInterface(BTCInterface):
    ...
    def findTxnByHash(self, txid_hex: str):
        ...
        if (
            'confirmations' in rv
            and rv['confirmations'] >= self.blocks_confirmed
        ):
    ...
    def getLockTxHeight(
        ...
        if not txid:
            ...
            for tx in txns:
                if (
                    self.make_int(tx['amount']) == bid_amount
                    and tx['category'] == 'send'
                    and tx['address'] == dest_address
                ):
                    txid = bytes.fromhex(tx['txid'])
                    break
        ...
        try:
            ...
            rv = {
                'depth': (
                    0 if 'confirmations' not in tx else tx['confirmations']
                ),
                'height': block_height,
            }
        ...
        if find_index:
            ...
            rv['index'] = find_vout_for_address_from_txobj(
                tx_obj, dest_address
            )
    ...
    def genScriptLockTxScript(
        self, ci, Kal: bytes, Kaf: bytes, **kwargs
    ) -> CScript:
        mining_fee: int = (
            kwargs['mining_fee'] if 'mining_fee' in kwargs else 1000
        )
        out_1: bytes = kwargs['out_1']
        out_2: bytes = kwargs['out_2']
        public_key: bytes = (
            kwargs['public_key'] if 'public_key' in kwargs else Kal
        )
    ...
    def addressToLockingBytecode(self, address: str) -> bytes:
        return (
            b'\x76\xa9\x14'
            + bytes(Address.from_string(address).payload)
            + b'\x88\xac'
        )
    ...
    def scriptToP2SH32LockingBytecode(
        self, script: Union[bytes, str]
    ) -> bytes:
        return CScript(
            [
                OP_HASH256,
        ...
        **kwargs,
        ...
        tx.vin.append(
            CTxIn(
                COutPoint(tx_lock_id_int, locked_n),
                nSequence=(
                    kwargs['timelock'] if 'timelock' in kwargs else lock1_value
                ),
                scriptSig=self.getScriptScriptSig(script_lock, None),
            )
        )
        tx.vout.append(
            self.txoType()(locked_coin, self.getScriptDest(refund_script))
        )

        pay_fee = (
            kwargs['mining_fee'] if 'mining_fee' in kwargs else tx_fee_rate
        )
        tx.vout[0].nValue = locked_coin - pay_fee
    ...
    def verifySCLockTx(
        ...
        **kwargs,
        ...
        self._log.info('Verifying lock tx: {}.'.format(b2h(txid)))
            ...
            tx.nLockTime == 0, 'Bad nLockTime'
        )  # TODO match txns created by cores
    ...
    def verifySCLockRefundTx(
        ...
        **kwargs,
        ...
        self._log.info('Verifying lock refund tx: {}.'.format(b2h(txid)))
            ...
            tx.vin[0].scriptSig
            == self.getScriptScriptSig(prevout_script, None),
    ...
    def verifySCLockRefundSpendTx(
        ...
        **kwargs,
        ...
        self._log.info('Verifying lock refund spend tx: {}.'.format(b2h(txid)))
            ...
            tx.vin[0].scriptSig
            == self.getScriptScriptSig(prevout_script, bytes(73)),
            'Input scriptsig mismatch',
        )
        ensure(
    ...
    def extractLeaderSig(self, tx_bytes: bytes) -> bytes:
        tx = self.loadTx(tx_bytes)
        signature, _, _, _, _, _ = (
            self.extractScriptLockScriptValuesFromScriptSig(
                tx.vin[0].scriptSig
            )
        )
        return signature
