            ...
            if have_data_entry(form_data, "apply_general"):
                ...
                data = {
                    "session_timeout_minutes": int(
                        get_data_entry_or(form_data, "session_timeout_minutes", "60")
                    ),
                }
                swap_client.editGeneralSettings(data)
                ...
                if action == "disable":
                    ...
                    if "*" in allowed_hosts and not swap_client.settings.get(
                        ...
                        messages.append("Web UI password disabled.")
                else:
                    ...
                    if new_password == "":
                        ...
                        if auth_enabled:
                            ...
                            messages.append("Web UI password changed.")
                        else:
                            _, cookie_header = self._create_session()
                            extra_headers.append(cookie_header)
                            messages.append("Web UI password enabled.")
